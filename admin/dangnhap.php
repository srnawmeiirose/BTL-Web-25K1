...
if (isset($_SESSION['username'])) {
    ...
    if ($rowCheck['vai_tro_id'] == 1) {
        header("Location:danhsachdkygoi.php"); // quản trị viên
    } else {
        header("Location:index.php?page_layout=trangchu"); // admin hoặc các vai trò khác
    }
    exit;
}
...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($row['vai_tro_id'] == 1) {
            header("Location: danhsachdkygoi.php"); // hội viên
        } else {
            header("Location:index.php?page_layout=trangchu"); // admin
        }
        exit();
    } else {
